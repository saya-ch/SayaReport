...
2. **检索**：对五大板块逐个推进，执行多轮检索与多渠道交叉验证
3. **设计**：优先使用内置 PPT 设计 skill（guizang-ppt-skill）规划版式与配色，生成图片/视频素材（不可用时以高质量动画降级），确保报告高品质呈现
...
| skills/ | 内置技能库：guizang-ppt-skill（PPT 设计，阶段三优先使用）+ ui-ux-pro-max（UI/UX 设计情报库）+ gsap-skills（GSAP 动画技能库） |
| LICENSE | MIT 开源许可 |
