...
6. **确认内置技能库（为阶段三铺垫）**：确认仓库 `skills/` 目录已包含 `guizang-ppt-skill/`（PPT 设计技能库，阶段三**优先使用**）、`ui-ux-pro-max/`（UI/UX 设计情报库）与 `gsap-skills/`（GSAP 动画技能库）。阶段三设计报告时必须阅读并使用它们（见 `PHASE-3.md`）。
